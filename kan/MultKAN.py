...
class MultKAN(nn.Module):
    ...
    act_fun : KANLayer列表
        激活函数层集合
    symbolic_fun: Symbolic_KANLayer列表
        符号函数层集合
    depth : int
        KAN网络深度 
    width : 列表
        每层神经元数量。
        不使用乘法节点时，[2,5,5,3]表示2维输入、3维输出，含2层5个隐藏神经元。
        使用乘法节点时，[2,[5,3],[5,1],3]表示除了[2,5,53]的KAN外，第1(2)层还有3(1)个乘法节点。
    mult_arity : int 或 嵌套整数列表
        每个乘法节点的元数(要相乘的数字个数)
    base_fun : 函数
        残差函数b(x)。激活函数形式为 phi(x) = sb_scale*b(x) + sp_scale*spline(x)
    symbolic_enabled : bool
        若为False则不计算符号分支(节省时间)。默认True。
    width_in : 列表
        每层的输入神经元数 
    width_out : 列表  
        每层的输出神经元数  
    base_fun_name : str  
        基函数b(x)名称  
    grip_eps : float  
        在均匀网格和自适应网格(基于样本分位数)之间插值的参数  
    node_bias : 一维torch.float列表  
    node_scale : 一维torch.float列表  
    subnode_bias : 一维torch.float列表  
    subnode_scale : 一维torch.float列表  
    affine_trainable : bool  
        指示仿射参数是否可训练(node_bias等)  
    sp_trainable : bool  
        指示样条整体幅度是否可训练  
    sb_trainable : bool  
        指示基函数整体幅度是否可训练  
    save_act : bool  
        指示前向传播中是否保存中间激活值  
    node_scores : None或一维torch.float列表  
        节点归因分数  
    edge_scores : None或二维torch.float列表   
        边归因分数   
    subnode_scores: None或一维torch.float列表   
    子节点归因分数   
    cache_data: None或二维torch.float   
    缓存的输入数据   
    acts: None或二维torch.float列表   
    节点上的激活值   
    auto_save: bool   
    模型修改后是否自动保存检查点   
    state_id: int   
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun='silu', symbolic_enabled=True, affine_trainable=False, grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, seed=1, save_act=True, sparse_init=False, auto_save=True, first_init=True, ckpt_path='./model', state_id=0, round=0, device='cpu'):
        ...
        -----
            width : int 列表
            不含乘法节点时: :math:`[n_0, n_1, .., n_{L-1}]` 指定每层神经元数（含输入/输出）
            含乘法节点时: :math:`[[n_0,m_0=0], [n_1,m_1], .., [n_{L-1},m_{L-1}]]` 指定每层加法/乘法节点数（含输入/输出）

            grid : int
            网格区间数。默认 3。
            k : int
            分段多项式阶数。默认 3。
            mult_arity : int 或 int 列表
            每个乘法节点的元数（相乘的输入数）
            noise_scale : float
            样条初始噪声幅度。
            base_fun : str
            残差函数 b(x)。默认 'silu'
            symbolic_enabled : bool
            是否计算符号分支（True）或跳过（False）（提升效率）。默认 True。
            affine_trainable : bool
            仿射参数是否可训练。包括 node_scale, node_bias, subnode_scale, subnode_bias
            grid_eps : float
            grid_eps=1 时网格均匀；grid_eps=0 时按样本分位数划分。0 < grid_eps < 1 为插值。
            grid_range : list/np.array, 形状 (2,)
            网格范围。默认 [-1,1]。若 fit(update_grid=True)（默认）则该参数不重要。
            sp_trainable : bool
            若为 True，scale_sp 可训练。默认 True。
            sb_trainable : bool
            若为 True，scale_base 可训练。默认 True。
            device : str
            设备
            seed : int
            随机种子
    ...
    def plot(self, folder="./figures", beta=3, metric='backward', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0):
            ...
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l+1]):
                    ...
                    if alpha_mask == 1:
                        ...
                        plt.gca().patch.set_edgecolor("#114A86")  # 深灰色边框
                    else:
                        plt.gca().patch.set_edgecolor('white')
                        plt.gca().patch.set_edgecolor("#1663af") #与背景色一致
        ...
        fig, ax = plt.subplots(figsize=(20 * scale, 20 * scale * (neuron_depth - 1) * (y0+z0)))
        ...
        # plot scatters and lines绘制散点图和线条
        for l in range(neuron_depth):
            ...
            for i in range(n):
             ...
             # scatters
            for i in range(n):
                plt.scatter(1 / (2 * n) + i / n, l * (y0+z0), s=min_spacing ** 2 * 10000 * scale ** 2,cmap='coolwarm',alpha=0.9, edgecolors='grey')                
    ...
    def evaluate(self, dataset):
        ...
        #采用均方差作为评价指标
        evaluation['test_loss']=evaluation['test_loss'] **2
